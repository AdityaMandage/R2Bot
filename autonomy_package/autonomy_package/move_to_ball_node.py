...
class BallMoverNode(Node):
    ...
    def move_towards_ball(self, detection):
        ...
        # Adjust the linear velocity based on the y_center
        stopping_region_start = image_height - 240  # 240 pixels from the bottom

        if y_center >= stopping_region_start:
            twist.linear.x = 0.0  # Stop if the ball is in the stopping region
            self.get_logger().info('Ball in stopping region, stopping the bot.')
        else:
            twist.linear.x = 0.4  # Move forward if the ball is not in the stopping region
